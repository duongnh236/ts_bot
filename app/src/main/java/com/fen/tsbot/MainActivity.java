...
public class MainActivity extends Activity {
    ...
    private final BroadcastReceiver resultReceiver = new BroadcastReceiver() {
        public void onReceive(Context c, Intent i) { String raw=i.getStringExtra("json");status.setText(raw);try{JSONObject r=new JSONObject(raw);String msg=r.optString("message",raw);status.setText(msg);boolean error=!r.optBoolean("ok");if(loginAllPending){loginAllAcknowledged=true;if(error){loginAllPending=false;accountManagerView.clearLoginPending("");}refreshAccountsDashboard();updateTeamActionButtons();}else if(error&&accountManagerView!=null)accountManagerView.clearLoginPending("");if(accountManagerView!=null)accountManagerView.setActionMessage(msg,error);Toast.makeText(MainActivity.this,msg,Toast.LENGTH_LONG).show();}catch(Exception e){if(accountManagerView!=null)accountManagerView.setActionMessage(raw,true);} }
    };
    ...
    private View buildUi() {
        ...
        root.addView(section("TỔNG QUAN TEAM"));
...
private void showLeaderSwitchDialog(){List<String> names=new ArrayList<>(),ids=new ArrayList<>();for(int i=0;i<bottomAccounts.length();i++){JSONObject a=bottomAccounts.optJSONObject(i);if(a!=null&&a.optBoolean("online")){ids.add(a.optString("user"));names.add(a.optString("name",a.optString("user"))+(a.optBoolean("leader")?" • LEADER HIỆN TẠI":""));}}if(ids.isEmpty()){Toast.makeText(this,"Cần ít nhất 1 account online",Toast.LENGTH_LONG).show();return;}new AlertDialog.Builder(this).setTitle("Chọn leader mới").setItems(names.toArray(new String[0]),(d,index)->new AlertDialog.Builder(this).setTitle("Đổi leader sang "+names.get(index)).setMessage("Nếu chưa có party: chọn trực tiếp, không cần ACC1 online. Nếu đã có party: chờ hết trận → leader cũ kéo team về SAFE → giải tán party → leader mới mời lại team. Không logout account. Nếu đang farm sẽ tiếp tục bãi đã chọn sau khi đổi xong.").setNegativeButton("HỦY",null).setPositiveButton("ĐỔI LEADER",(confirm,w)->switchOnlineLeader(ids.get(index))).show()).setNegativeButton("ĐÓNG",null).show();}
    ...
    private void startOne(int slot,String user,String pass){try{if(user==null||user.trim().isEmpty())throw new Exception("Chưa nhập username");if(pass==null||pass.isEmpty())throw new Exception("Chưa nhập password");if(servers.isEmpty()||maps.isEmpty())throw new Exception("Danh sách server chưa nạp xong, chờ vài giây rồi thử lại");users[slot].setText(user.trim());passes[slot].setText(pass);enabled[slot].setChecked(true);JSONObject p=buildPayload(true,true).put("only_user",user.trim());accountManagerView.markLoginPending(slot);startForegroundService(new Intent(this,BotService.class).setAction(BotService.START).putExtra("payload",p.toString()));String msg="Đang đăng nhập riêng "+user+" qua "+servers.get(serverSpinner.getSelectedItemPosition()).optString("label");accountManagerView.setActionMessage(msg,false);Toast.makeText(this,msg,Toast.LENGTH_SHORT).show();}catch(Exception e){accountManagerView.clearLoginPending(user);String msg="Lỗi đăng nhập: "+e.getMessage();if(accountManagerView!=null)accountManagerView.setActionMessage(msg,true);Toast.makeText(this,msg,Toast.LENGTH_LONG).show();}}
